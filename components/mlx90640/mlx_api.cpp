...
namespace esphome
{
    namespace mlx90640
    {
        ...
        int MLX90640_API::MLX90640_GetFrameData(uint8_t slaveAddr, uint16_t *frameData) {
            ...
            dataReady = 0;
            while (dataReady == 0) {
                error = this->driver->MLX90640_I2CRead(slaveAddr, 0x8000, 1, &statusRegister);
                if (error != 0) {
                    return error;
                }
            ...
            while (dataReady != 0 && cnt < 5) {
                error = this->driver->MLX90640_I2CWrite(slaveAddr, 0x8000, 0x0030);
                if (error == -1) {
                    return error;
                }

                error = this->driver->MLX90640_I2CRead(slaveAddr, 0x0400, 832, frameData);
                if (error != 0) {
                    return error;
                }
            ...
            error          = this->driver->MLX90640_I2CRead(slaveAddr, 0x800D, 1, &controlRegister1);
        ...
        int MLX90640_API::MLX90640_SetResolution(uint8_t slaveAddr, uint8_t resolution) {
            ...
            if (error == 0) {
                ...
                error = this->driver->MLX90640_I2CWrite(slaveAddr, 0x800D, value);
            }
