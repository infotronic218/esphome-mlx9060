  ...
  for (fan::Fan *obj : App.get_fans()) {
    ...
    if (request->method() == HTTP_GET) {
      ...
      if (request->hasParam("speed")) {
        String speed = request->getParam("speed");
      }
      if (request->hasParam("speed_level")) {
        String speed_level = request->getParam("speed_level");
      ...
      if (request->hasParam("oscillation")) {
        String speed = request->getParam("oscillation");
  ...
  for (light::LightState *obj : App.get_lights()) {
    ...
    if (request->method() == HTTP_GET) {
      ...
      if (request->hasParam("brightness"))
        call.set_brightness(request->getParam("brightness").toFloat() / 255.0f);
      if (request->hasParam("r"))
        call.set_red(request->getParam("r").toFloat() / 255.0f);
      if (request->hasParam("g"))
        call.set_green(request->getParam("g").toFloat() / 255.0f);
      if (request->hasParam("b"))
        call.set_blue(request->getParam("b").toFloat() / 255.0f);
      if (request->hasParam("white_value"))
        call.set_white(request->getParam("white_value").toFloat() / 255.0f);
      if (request->hasParam("color_temp"))
        call.set_color_temperature(request->getParam("color_temp").toFloat());

      if (request->hasParam("flash")) {
        float length_s = request->getParam("flash").toFloat();
        call.set_flash_length(static_cast<uint32_t>(length_s * 1000));
      }

      if (request->hasParam("transition")) {
        float length_s = request->getParam("transition").toFloat();
        call.set_transition_length(static_cast<uint32_t>(length_s * 1000));
      }

      if (request->hasParam("effect")) {
        const char *effect = request->getParam("effect").c_str();
      ...
      if (request->hasParam("transition")) {
        auto length = (uint32_t) request->getParam("transition").toFloat() * 1000;
...
void WebServer::handle_climate_request(AsyncWebServerRequest *request, const UrlMatch &match) {
  for (auto *obj : App.get_climates()) {
    ...
    if (request->hasParam("target_temperature_high")) {
      String value = request->getParam("target_temperature_high");
      optional<float> value_f = parse_number<float>(value.c_str());
      if (value_f.has_value())
    ...
    if (request->hasParam("target_temperature")) {
      String value = request->getParam("target_temperature");
      optional<float> value_f = parse_number<float>(value.c_str());
      if (value_f.has_value())
